...
class DatabaseOperations(generic.DatabaseOperations):
    ...
    def _get_full_table_description(self, connection, cursor, table_name):
        ...
        return [{'name': field[1],
                 'type': field[2],
                 'null_ok': not field[3],
                 'dflt_value': self._default_value_workaround(default=field[4], type=field[2]),
    ...
    def _default_value_workaround(self, default, type=None):
        if type and 'varchar' in type:
            default = "'%s'" % default
        elif type and 'integer' in type:
            default = "%s" % default
        if default == True:
            default = '1'
        elif default == False:
